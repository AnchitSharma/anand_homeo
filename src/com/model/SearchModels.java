...
public List<List<String>> selectData(String tableName, List<String> columns,
		Map<String, Object> where,List<String> grpdate){
...
//	select date ,sum(payment) from invoice_table group by date;
		...
		for(String s2:where.keySet()) {
			if (s2.contains("between")) {
				//select * from patient_table where reg_date between '30-01-2018' and '31-01-2018';
				sql.append(where.get(s2));
				sql.append(" and ");
			}
...
//			//System.out.println("where is empty detecetd");
		...
		//System.out.println(s1);
//		select date ,sum(payment) from invoice_table group by date;
//		select date , sum(medcine_price) from medicine_order_table where medicine_id like  'Oiling%' group by date;
		if (!grpdate.isEmpty()) {
			/*if(grpdate.get(0).contains(Constants.group_by)){
			sql = new StringBuilder(s1);
		sql.append(" group by ");
...
public List<List<String>> selectData(String tableName, List<String> columns,
		Map<String, Object> where,String grpdate){
...
//			where.put("cancel_doctor", 1); iwill have filter the data over my end
			...
			if (!selectdata.isEmpty()) {
				for(List<String> strList:selectdata) {
					...
					med = new MedicineModel(strList.get(0),strList.get(1) , strList.get(2),
							"",strList.get(4) , strList.get(5), strList.get(7) ,  strList.get(6));
					list.add(med);
				}
			}
		}
